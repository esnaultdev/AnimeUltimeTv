 ...
 */
class ExoPlayerAdapter(context: Context) : PlayerAdapter(), ExoPlayer.EventListener {
    ...
    override fun getDuration(): Long = (if (mInitialized) mPlayer.duration else -1).toLong()

    override fun getCurrentPosition(): Long = (if (mInitialized) mPlayer.currentPosition else -1).toLong()
    ...
    override fun isPrepared(): Boolean = mInitialized && (mSurfaceHolderGlueHost == null || mHasDisplay)
    ...
    override fun onPlayerError(error: ExoPlaybackException) {
        callback.onError(this@ExoPlayerAdapter, error.type,
                context.getString(R.string.mediaPlayer_error,
                        error.type,
                        error.rendererIndex))
    }
